<?php

require 'config/database.php';

if(isset($_POST['submit'])){
    //get form data
    $title =filter_var($_POST['title'],FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    $description=filter_var($_POST['description'],FILTER_SANITIZE_FULL_SPECIAL_CHARS);

    if(!$title){
        $_SESSION['add-tag'] = "Enter title";

    }elseif(!$description){
        $_SESSION['add-tag']="Enter description";
    
    }
    // redirect back to add category page if there was invalid input
    if(isset($_SESSION['add-tag'])){
        $_SESSION['add-tag-data'] = $_POST;
        header('location: ' . ROOT_URL . 'admin/add-tag.php');
        die();
    }else{
        // insert category into database
    $query = "INSERT INTO tags  (title , description) VALUES ('$title','$description')";
        $result = mysqli_query($connection, $query);
        if(mysqli_errno($connection)){
            $_SESSION['add-tag'] = "Couldn't add tag";
            header('location: ' . ROOT_URL . 'admin/add-tag.php');
            die();
        }else{
            $_SESSION['add-tag-success'] = "Tag $title tag added successfully";
            header('location: ' . ROOT_URL . 'admin/manage-tags.php');
            
        }
    }
}